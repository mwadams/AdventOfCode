namespace AdventOfCode.Runner.Year2023.Day19
{
    ...
    public class DayRunner : IDay<DayRunner>
    {
            ...
            where TFormatter : IResultFormatter
        {
            ...
            WorkflowStep initialStep = BuildWorkflowSteps(lines, steps);
            ...
            foreach(ReadOnlySpan<char> line in lines.AsSpan()[startLine..])
            {
                ...
                State state = initialStep(State.For(part, steps));
                if (state.Result == Result.Accept)
                {
        ...
        private static Part GetPart(ReadOnlySpan<char> line)
        {
            int start = 1;
        ...
        private static WorkflowStep BuildWorkflowSteps(ReadOnlySpan<string> lines, Dictionary<string, WorkflowStep> steps)
        {
        ...
        private delegate State WorkflowStep(State state);
